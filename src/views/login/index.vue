...
import { mapActions } from 'vuex' // 引入 vuex 的辅助函数
export default {
  name: 'Login',
  data() {
     ...
     this.$refs.loginForm.validate(async isOK => {
      if(isOK) {
        try{
          ...
          // 应该是登录成功后
          // async 标记的函数实际上是一个 promise 对象
          // await 下面的代码 都是成功执行的代码
        this.$router.push('/')
        } catch(err) {
          console.log(err)
        } finally {
          // 不论执行 try 还是 catch 都会执行 finally
          this.loading = false
        }
      }
     })
      // ref 可以获取到一个元素的 dom 对象
      // ref 作用到组件上的时候，可以获取到该组件的实例(类似与 this )
    }
  }
}
</script>
